  ...
  useEffect(() => {
    const fetchUpcomingTours = async () => {
      try {
        ...
        const res = await apiClient.get<any>(TOURS_API.GET_UPCOMING_TOURS, {
        ...
        setError(err.message || "Failed to fetch upcoming tours");
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <>
      <div className="mx-auto max-w-7xl pb-24 sm:px-6 lg:px-8 mt-4 lg:mt-8">
        <div className="mb-8 flex flex-col gap-2">
          <div className="flex items-center gap-2 text-sm font-semibold text-emerald-600 uppercase tracking-widest">
            <span className="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-100">
              ✈️
            </span>
            {t("home.preparingToDepartEyebrow")}
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 md:text-4xl">
            {t("home.upcomingToursTitle") || "Upcoming Tours"}
          </h1>
          <p className="text-slate-600 max-w-2xl">
            {t("home.upcomingToursSubtitle")}
          </p>
        </div>
