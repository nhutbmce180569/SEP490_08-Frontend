    ...
    <div className="glass-card h-full p-5">
      ...
      <div className="mb-4 grid grid-cols-3 gap-3">
        {weather.avgMaxTempC != null && (
          <Stat icon={Thermometer} label={t("common.max")} value={`${weather.avgMaxTempC.toFixed(1)}°C`} />
        )}
        {weather.avgMinTempC != null && (
          <Stat icon={Thermometer} label={t("common.min")} value={`${weather.avgMinTempC.toFixed(1)}°C`} />
        )}
        {weather.totalRainMm != null && (
          <Stat icon={Droplets} label={t("ai.rain")} value={`${weather.totalRainMm.toFixed(1)} mm`} />
        )}
      </div>
