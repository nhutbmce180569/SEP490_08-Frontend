...
import type { CreateReviewRequest, Review, ReviewFilterParams, UpdateReviewRequest } from "../types/review";
...
export const useReview = () => {
  ...
  const fetchReviewsByTour = useCallback(async (tourId: number, params: ReviewFilterParams) => {
    setIsLoading(true);
    setError(null);
    try {
  ...
  const fetchReviewsForAdmin = useCallback(async (tourId: number, params: ReviewFilterParams) => {
    setIsLoading(true);
    setError(null);
    try {
      const response = await reviewService.getReviewsByTourAdmin(tourId, params);
