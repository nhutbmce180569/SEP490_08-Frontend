...
export const DeleteUserConfirm: React.FC = () => {
  ...
  if (isFetching) return <div className="flex justify-center p-10 text-slate-500">{t("auth.loadingUserDetails")}</div>;
  ...
  return (
    <div className="mx-auto max-w-2xl py-8">
      <button onClick={handleCancel} className="mb-6 flex items-center gap-2 text-sm font-semibold text-slate-500 transition-colors hover:text-slate-800">
        <ArrowLeft className="h-4 w-4" /> {t("auth.backToUsers")}
      </button>
      ...
      <div className="overflow-hidden rounded-2xl border border-rose-200 bg-white shadow-sm">
        <div className="flex items-start gap-4 border-b border-rose-100 bg-rose-50/50 px-6 py-5">
          ...
          <div>
            <h2 className="text-lg font-bold text-rose-700">{t("auth.deleteUserConfirmation")}</h2>
            <p className="mt-1 text-sm text-rose-600/90">
              {t("auth.deleteUserWarning")}
            </p>
          </div>
        </div>

        <div className="p-6">
          <div className="mb-4 text-sm font-bold text-slate-800">{t("auth.userDetailsToDelete")}</div>
      ...
      <LoadingOverlay isOpen={isDeleting} message={t("auth.deletingUser")} />
    </div>
  );
};
