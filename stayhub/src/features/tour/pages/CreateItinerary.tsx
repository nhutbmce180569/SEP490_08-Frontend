...
export const CreateItinerary: React.FC = () => {
  ...
  if (!tourId) {
    return <div className="p-10 text-center text-rose-500">Tour ID is missing from URL.</div>;
  }

  if (isTourLoading) {
    return <div className="p-10 text-center text-slate-500">Loading tour details...</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-4xl py-6">
      ...
      <div className="mb-6 space-y-4 border-b border-slate-200 pb-5">
        ...
        {missingDayNumbers && missingDayNumbers.length > 0 && (
          <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 mt-4">
            ...
            <p>Day {missingDayNumbers.join(", Day ")} is not present in this tour yet.</p>
          </div>
        )}
      </div>
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  <div className="sm:col-span-2">
                    <label className="mb-1.5 flex items-center gap-1.5 text-sm font-semibold text-slate-700">
                      <Type className="h-4 w-4 text-slate-400" /> Title <span className="text-rose-500">*</span>
                    </label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="sm:col-span-2">
                    <label className="mb-1.5 flex items-center gap-1.5 text-sm font-semibold text-slate-700">
                      <FileText className="h-4 w-4 text-slate-400" /> Description <span className="text-rose-500">*</span>
                    </label>
                    <textarea
                      required
                      rows={3}
      ...
      <LoadingOverlay isOpen={isSubmitting} message="Saving all itineraries..." />
