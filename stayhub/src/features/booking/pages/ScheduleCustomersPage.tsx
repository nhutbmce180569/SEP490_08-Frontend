...
export const ScheduleCustomersPage: React.FC = () => {
  ...
  const handleSearch = async () => {
    ...
    if (!id || Number.isNaN(id) || id <= 0) {
      showError(t('booking.invalidScheduleId'));
      return;
    }
    ...
    <div className="mx-auto max-w-6xl px-4 py-6">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-extrabold text-slate-900">{t('booking.scheduleCustomerCenter')}</h1>
          <p className="mt-2 max-w-2xl text-sm text-slate-600">{t('booking.scheduleCustomerDesc')}</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <input
            ...
            placeholder={t('booking.enterScheduleId')}
      ...
      <div className="mb-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-5 flex items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">{t('booking.assignedSchedules')}</h2>
            <p className="text-sm text-slate-500">{t('booking.selectScheduleHint')}</p>
          </div>
          {isScheduleLoading ? (
            <span className="text-sm text-slate-500">{t('booking.loadingSchedules')}</span>
          ) : null}
        </div>

        {assignedSchedules.length === 0 && !isScheduleLoading ? (
          <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-6 text-sm text-slate-500">
            {t('booking.noAssignedSchedules')}
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm text-slate-700">
              <thead className="bg-slate-50 text-slate-700">
                <tr>
                  <th className="border-b border-slate-200 px-4 py-3 font-semibold">{t('booking.schedule')}</th>
                  <th className="border-b border-slate-200 px-4 py-3 font-semibold">{t('booking.dates')}</th>
                  <th className="border-b border-slate-200 px-4 py-3 font-semibold">{t('booking.role')}</th>
                  <th className="border-b border-slate-200 px-4 py-3 font-semibold">{t('common.action')}</th>
                </tr>
              </thead>
              <tbody>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-sm font-semibold text-slate-500">
                ...
                {t('booking.scheduleNumber', { id: currentScheduleId })}
              </div>
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="overflow-x-auto">
          <table className="min-w-full border-separate border-spacing-0 text-left text-sm text-slate-700">
            <thead className="bg-slate-50 text-slate-700">
              <tr>
                <th className="border-b border-slate-200 px-4 py-4 font-semibold">{t('common.nameLabel')}</th>
                <th className="border-b border-slate-200 px-4 py-4 font-semibold">{t('booking.genderLabel')}</th>
                <th className="border-b border-slate-200 px-4 py-4 font-semibold">{t('common.dateOfBirth')}</th>
                <th className="border-b border-slate-200 px-4 py-4 font-semibold">{t('common.phone')}</th>
                <th className="border-b border-slate-200 px-4 py-4 font-semibold">{t('common.ticket')}</th>
                <th className="border-b border-slate-200 px-4 py-4 font-semibold">{t('common.order')}</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <td className="border-b border-slate-200 px-4 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-slate-500">{customer.idCard || t('booking.noIdCard')}</div>
                      </div>
                    </div>
                  </td>
        ...
        {isLoading ? (
          <div className="border-t border-slate-200 px-4 py-4 text-sm text-slate-500">{t('booking.loadingCustomers')}</div>
        ) : null}
      </div>
    </div>
  );
