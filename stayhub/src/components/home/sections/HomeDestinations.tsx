...
const DestinationCard = ({ dest, onClick }: { dest: any, onClick: () => void }) => {
  const { t } = useTranslation();
  const { translatedText } = useDynamicTranslation(dest.name);
        ...
        <div className="grid auto-rows-[minmax(140px,1fr)] grid-cols-2 gap-3 md:gap-4 lg:grid-cols-4 lg:grid-rows-2 lg:min-h-[440px]">
          {dynamicDestinations.map((dest) => (
            <DestinationCard 
              key={dest.name} 
              dest={dest} 
              onClick={() => navigate(`${PATH.PUBLIC.TOUR_SEARCH}?searchTerm=${encodeURIComponent(dest.name)}`)} 
            />
          ))}
        </div>
      )}
