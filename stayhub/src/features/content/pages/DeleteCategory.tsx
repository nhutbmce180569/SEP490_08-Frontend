...
export const DeleteCategoryConfirm: React.FC = () => {
  ...
  const { category, isFetching, fetchError, isDeleting, handleConfirmDelete, handleCancel } =
    useDeleteCategory();
  ...
  return (
    <div className="mx-auto max-w-2xl py-8">
      ...
      >
        <ArrowLeft className="h-4 w-4" /> {t("content.backToCategories")}
      </button>
      ...
      <div className="overflow-hidden rounded-2xl border border-rose-200 bg-white shadow-sm">
        <div className="flex items-start gap-4 border-b border-rose-100 bg-rose-50/50 px-6 py-5">
          ...
          <div>
            <h2 className="text-lg font-bold text-rose-700">{t("content.deleteCategoryConfirm")}</h2>
            <p className="mt-1 text-sm text-rose-600/90">{t("content.deleteCategoryWarning")}</p>
          </div>
        </div>

        <div className="p-6">
          <div className="mb-4 text-sm font-bold text-slate-800">
            {t("content.categoryDetailsToDelete")}
          </div>
          <div className="flex flex-col gap-5 rounded-xl border border-slate-200 bg-slate-50 p-4 sm:flex-row">
            {category.iconUrl ? (
              <img
                src={getImg(category.iconUrl)}
                alt={category.name}
                className="h-24 w-24 rounded-lg border border-slate-200 object-cover shadow-sm"
              />
            ) : (
              <div className="flex h-24 w-24 shrink-0 items-center justify-center rounded-lg bg-slate-200 text-slate-400 shadow-sm">
            ...
            <div className="flex-1 space-y-2.5">
              ...
              <div className="flex flex-col gap-2 text-sm text-slate-600">
                ...
                <div className="mt-1 line-clamp-2">
                  {category.description || t("content.noDescriptionProvided")}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-end gap-3 border-t border-slate-100 bg-slate-50/50 px-6 py-4">
          ...
          <ActionButton
            ...
            className="gap-2 px-5 py-2.5 text-sm !border-rose-600 !bg-rose-600 !text-white hover:!bg-rose-700"
          >
            <Trash2 className="h-4 w-4" /> {t("content.yesDeleteCategory")}
          </ActionButton>
        </div>
      </div>
      <LoadingOverlay isOpen={isDeleting} message={t("content.deletingCategory")} />
    </div>
  );
};
